...
- [ ] **5.4 一覧ページの実装 (Works / Composers Index)**
    - [ ] **[仕様策定]** フィルタリング・ソート仕様の策定
        ...
        - [ ] **サムネイル自動解決ロジック** の定義（YouTube `maxresdefault.jpg` 優先、404時に `hqdefault.jpg` へフォールバックするロジック）
    - [ ] **[実装]** 一覧ページと自動サムネイル
        - [ ] `Works`/`Composers` 一覧のグリッドレイアウト実装
        - [ ] **Default YouTube Artwork** ロジックの実装（解像度優先順位付き自動適用）
        - [ ] ローディング中のスケルトン表示の実装
...
- [ ] **6.2 高品質な譜例リポジトリの選定とシステマチックな抽出の検討**
    - [ ] **[意図]** 譜例をゼロからAIで生成する精度懸念への対応
    - [ ] **Verovio (MusicXML/MEI)** の検討
    - [ ] 既存の譜例から「聴きどころの数小節（Snippet）」をAIで抽出・変換する戦略の策定

- [ ] **6.3 翻訳エージェント (Translator Agent)**
...
- [ ] **6.6 品質保証エージェント (Quality Assurance Agent)**
    - [ ] **[仕様策定]** 自動巡回チェック項目の定義
        - [ ] **事実確認**: 作品番号、成立年、調性が公式DBと一致するか
        - [ ] **リンク生存**: YouTube動画の削除や著作権失効の自動チェック
        - [ ] **音楽的一貫性**: 解説文の内容と提示された譜例の整合性チェック
    - [ ] **[実装]** QA巡回システムの構築
...
- [ ] **9.4 Monetization Strategy**
    - [ ] **[実装]** 初回ローンチ時のマネタイズ対応
        - [ ] アフィリエイトリンク（楽譜・音源）の自動生成とコンテキスト配置
        - [ ] Apple Music / Spotify ウィジェットの自動挿入による体験向上と収益化
