...
export const SearchBox = () => {
    ...
    useEffect(() => {
        async function loadPagefind() {
            if (typeof window.pagefind === 'undefined') {
                try {
                    // Function to load script dynamically avoids bundler trying to resolve it
                    ...
                    console.warn('Pagefind script could not be loaded (likely not generated yet)', e);
                }
            } else {
                setIsLoaded(true);
            }
    ...
    const handleSearch = async (q: string) => {
        setQuery(q);
        if (q.length > 1 && isLoaded && window.pagefind) {
